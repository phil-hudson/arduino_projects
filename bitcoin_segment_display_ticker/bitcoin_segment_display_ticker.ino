...
#include <ArduinoWebsockets.h>  // https://github.com/pablorodiz/Arduino-Websocket
...
void connect(boolean reconnect) {
  ...
  ws.setInsecure();
  if (ws.connect(String(exchange.wsproto) + String(exchange.host) + String(exchange.url))) {
...
//  ws.connect(String(exchange.wsproto + exchange.host + exchange.url));
...
void onMessageCallback(WebsocketsMessage message) {
      ...
      if (message.data().length() > 0) {
        ...
        JsonArray& rootarray = jsonBuffer.parseArray(message.data());
