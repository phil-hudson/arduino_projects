...
func (d *Deej) run() {
	...
	go func() {
		...
		// todo this doesnt need to be every second but delay until d.serial.connected === true ?
		tick := time.Tick(10000 * time.Millisecond)
		for range tick {
			// dispatch current time to arduino
			go func() {
				dt := time.Now()
				_, offset := dt.Local().Zone()
				message := "T" + strconv.FormatInt(dt.Unix()+int64(offset), 10) + "\r\n"
				n, err := d.serial.conn.Write([]byte(message))
				if n > 0 {
					d.logger.Debug("Successful time sync: " + message)
				}
				if err != nil {
					d.logger.Warnw("Failed to sync time", "error", err)
				}
			}()
		}
