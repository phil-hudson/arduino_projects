		...
		for _, resolvedTarget := range resolvedTargets {
			...
			for _, session := range sessions {
				if session.GetVolume() != event.PercentValue {
					// handle the incremental difference
					newVol := session.GetVolume()
					fmt.Println(newVol)
					newVol += event.PercentValue

					if newVol > 100 {
						m.logger.Info("Target vol exceeded 100, capping to 100")
						newVol = 100
					}
					if newVol < 0 {
						m.logger.Info("Target vol below 0, capping to 0")
						newVol = 0
					}
					if err := session.SetVolume(newVol); err != nil {
