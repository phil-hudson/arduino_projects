...
func (d *Deej) run() {
	...
	go func() {
    	for range ticker.C {
			...
			sessions, _ := d.sessions.get("master") //d.sessions.get("discord.exe")
						// iterate all matching sessions and adjust the volume of each one
						for _, session := range sessions {

							n, err := d.serial.conn.Write([]byte(fmt.Sprintf("%f", session.GetVolume())))
							// fmt.Printf(d.sessions.m["master"].(Session))
